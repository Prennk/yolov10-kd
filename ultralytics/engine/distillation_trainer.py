...
from ultralytics.distiller import CriterionKD
...
class BaseDistillationTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None, teacher_model=None):
        """
    ...
    def distill(self):
        ...
        if world_size > 1 and "LOCAL_RANK" not in os.environ:
            ...
            criterion_kd = CriterionKD(temperature=4)
            self._do_distill(world_size, criterion_kd)
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                with autocast(self.amp):
                    ...
                    self.loss_yolo, self.loss_items, pred_scores, self.pred_distri = self.model(batch)
                    with torch.no_grad():
                        self.loss_t, self.loss_items_t, pred_scores_t, self.pred_distri_t = self.teacher_model.model(batch)
                    self.loss = criterion_kd(self.pred_distri, self.pred_distri_t)
